...
namespace ProjEMR.Services
{
    ...
    public interface IVisitTypeService
    {
        ...
        Task<List<VisitType>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc");

        /// <summary>Adds a new visittype</summary>
        /// <param name="model">The visittype data to be added</param>
        /// <returns>The result of the operation</returns>
        Task<Guid> Create(VisitType model);
        ...
        /// <returns>The result of the operation</returns>
        Task<bool> Update(Guid id, VisitType updatedEntity);

        /// <summary>Updates a specific visittype by its primary key</summary>
        /// <param name="id">The primary key of the visittype</param>
        /// <param name="updatedEntity">The visittype data to be updated</param>
        /// <returns>The result of the operation</returns>
        Task<bool> Patch(Guid id, JsonPatchDocument<VisitType> updatedEntity);
    ...
    public class VisitTypeService : IVisitTypeService
    {
        ...
        public VisitTypeService(ProjEMRContext dbContext, IFieldMapperService mapper)
        {
        ...
        public async Task<List<VisitType>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            var result = await GetVisitType(filters, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return result;
        }
        ...
        /// <param name="model">The visittype data to be added</param>
        /// <returns>The result of the operation</returns>
        public async Task<Guid> Create(VisitType model)
        {
            model.Id = await CreateVisitType(model);
            return model.Id;
        }
        ...
        public async Task<bool> Update(Guid id, VisitType updatedEntity)
        {
            await UpdateVisitType(id, updatedEntity);
            return true;
        }
        ...
        /// <returns>The result of the operation</returns>
        /// <exception cref="Exception"></exception>
        public async Task<bool> Patch(Guid id, JsonPatchDocument<VisitType> updatedEntity)
        {
            await PatchVisitType(id, updatedEntity);
            return true;
        }
        ...
        #region
        private async Task<List<VisitType>> GetVisitType(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            if (pageSize < 1)
            {
        ...
        private async Task<Guid> CreateVisitType(VisitType model)
        {
        ...
        private async Task UpdateVisitType(Guid id, VisitType updatedEntity)
        {
            _dbContext.VisitType.Update(updatedEntity);
            await _dbContext.SaveChangesAsync();
        }

        private async Task<bool> DeleteVisitType(Guid id)
        {
        ...
        private async Task PatchVisitType(Guid id, JsonPatchDocument<VisitType> updatedEntity)
        {
