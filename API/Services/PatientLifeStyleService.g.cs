...
namespace ProjEMR.Services
{
    ...
    public interface IPatientLifeStyleService
    {
        ...
        Task<List<PatientLifeStyle>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc");

        /// <summary>Adds a new patientlifestyle</summary>
        /// <param name="model">The patientlifestyle data to be added</param>
        /// <returns>The result of the operation</returns>
        Task<Guid> Create(PatientLifeStyle model);
        ...
        /// <returns>The result of the operation</returns>
        Task<bool> Update(Guid id, PatientLifeStyle updatedEntity);

        /// <summary>Updates a specific patientlifestyle by its primary key</summary>
        /// <param name="id">The primary key of the patientlifestyle</param>
        /// <param name="updatedEntity">The patientlifestyle data to be updated</param>
        /// <returns>The result of the operation</returns>
        Task<bool> Patch(Guid id, JsonPatchDocument<PatientLifeStyle> updatedEntity);
    ...
    public class PatientLifeStyleService : IPatientLifeStyleService
    {
        ...
        public PatientLifeStyleService(ProjEMRContext dbContext, IFieldMapperService mapper)
        {
        ...
        {
            var query = _dbContext.PatientLifeStyle.AsQueryable();
        ...
        public async Task<List<PatientLifeStyle>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            var result = await GetPatientLifeStyle(filters, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return result;
        }
        ...
        /// <param name="model">The patientlifestyle data to be added</param>
        /// <returns>The result of the operation</returns>
        public async Task<Guid> Create(PatientLifeStyle model)
        {
            model.Id = await CreatePatientLifeStyle(model);
            return model.Id;
        }
        ...
        public async Task<bool> Update(Guid id, PatientLifeStyle updatedEntity)
        {
            await UpdatePatientLifeStyle(id, updatedEntity);
            return true;
        }
        ...
        /// <returns>The result of the operation</returns>
        /// <exception cref="Exception"></exception>
        public async Task<bool> Patch(Guid id, JsonPatchDocument<PatientLifeStyle> updatedEntity)
        {
            await PatchPatientLifeStyle(id, updatedEntity);
            return true;
        }
        ...
        {
            await DeletePatientLifeStyle(id);
            return true;
        }
        #region
        private async Task<List<PatientLifeStyle>> GetPatientLifeStyle(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            if (pageSize < 1)
            {
        ...
        private async Task<Guid> CreatePatientLifeStyle(PatientLifeStyle model)
        {
        ...
        private async Task UpdatePatientLifeStyle(Guid id, PatientLifeStyle updatedEntity)
        {
            _dbContext.PatientLifeStyle.Update(updatedEntity);
            await _dbContext.SaveChangesAsync();
        }

        private async Task<bool> DeletePatientLifeStyle(Guid id)
        {
            var entityData = _dbContext.PatientLifeStyle.FirstOrDefault(entity => entity.Id == id);
            if (entityData == null)
            {
        ...
        private async Task PatchPatientLifeStyle(Guid id, JsonPatchDocument<PatientLifeStyle> updatedEntity)
        {
