...
namespace ProjEMR.Services
{
    ...
    public interface IBloodGroupService
    {
        ...
        Task<List<BloodGroup>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc");

        /// <summary>Adds a new bloodgroup</summary>
        /// <param name="model">The bloodgroup data to be added</param>
        /// <returns>The result of the operation</returns>
        Task<Guid> Create(BloodGroup model);
        ...
        /// <returns>The result of the operation</returns>
        Task<bool> Update(Guid id, BloodGroup updatedEntity);

        /// <summary>Updates a specific bloodgroup by its primary key</summary>
        /// <param name="id">The primary key of the bloodgroup</param>
        /// <param name="updatedEntity">The bloodgroup data to be updated</param>
        /// <returns>The result of the operation</returns>
        Task<bool> Patch(Guid id, JsonPatchDocument<BloodGroup> updatedEntity);
    ...
    public class BloodGroupService : IBloodGroupService
    {
        ...
        public BloodGroupService(ProjEMRContext dbContext, IFieldMapperService mapper)
        {
        ...
        {
            var query = _dbContext.BloodGroup.AsQueryable();
        ...
        public async Task<List<BloodGroup>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            var result = await GetBloodGroup(filters, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return result;
        }
        ...
        /// <param name="model">The bloodgroup data to be added</param>
        /// <returns>The result of the operation</returns>
        public async Task<Guid> Create(BloodGroup model)
        {
            model.Id = await CreateBloodGroup(model);
            return model.Id;
        }
        ...
        public async Task<bool> Update(Guid id, BloodGroup updatedEntity)
        {
            await UpdateBloodGroup(id, updatedEntity);
            return true;
        }
        ...
        /// <returns>The result of the operation</returns>
        /// <exception cref="Exception"></exception>
        public async Task<bool> Patch(Guid id, JsonPatchDocument<BloodGroup> updatedEntity)
        {
            await PatchBloodGroup(id, updatedEntity);
            return true;
        }
        ...
        #region
        private async Task<List<BloodGroup>> GetBloodGroup(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            if (pageSize < 1)
            {
        ...
        private async Task<Guid> CreateBloodGroup(BloodGroup model)
        {
        ...
        private async Task UpdateBloodGroup(Guid id, BloodGroup updatedEntity)
        {
            _dbContext.BloodGroup.Update(updatedEntity);
            await _dbContext.SaveChangesAsync();
        }

        private async Task<bool> DeleteBloodGroup(Guid id)
        {
            var entityData = _dbContext.BloodGroup.FirstOrDefault(entity => entity.Id == id);
            if (entityData == null)
            {
        ...
        private async Task PatchBloodGroup(Guid id, JsonPatchDocument<BloodGroup> updatedEntity)
        {
