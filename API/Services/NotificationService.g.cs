...
namespace ProjEMR.Services
{
    ...
    public interface INotificationService
    {
        ...
        Task<List<Notification>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc");

        /// <summary>Adds a new notification</summary>
        /// <param name="model">The notification data to be added</param>
        /// <returns>The result of the operation</returns>
        Task<Guid> Create(Notification model);
        ...
        /// <returns>The result of the operation</returns>
        Task<bool> Update(Guid id, Notification updatedEntity);

        /// <summary>Updates a specific notification by its primary key</summary>
        /// <param name="id">The primary key of the notification</param>
        /// <param name="updatedEntity">The notification data to be updated</param>
        /// <returns>The result of the operation</returns>
        Task<bool> Patch(Guid id, JsonPatchDocument<Notification> updatedEntity);
    ...
    public class NotificationService : INotificationService
    {
        ...
        public NotificationService(ProjEMRContext dbContext, IFieldMapperService mapper)
        {
        ...
        public async Task<List<Notification>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            var result = await GetNotification(filters, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return result;
        }
        ...
        /// <param name="model">The notification data to be added</param>
        /// <returns>The result of the operation</returns>
        public async Task<Guid> Create(Notification model)
        {
            model.Id = await CreateNotification(model);
            return model.Id;
        }
        ...
        public async Task<bool> Update(Guid id, Notification updatedEntity)
        {
            await UpdateNotification(id, updatedEntity);
            return true;
        }
        ...
        /// <returns>The result of the operation</returns>
        /// <exception cref="Exception"></exception>
        public async Task<bool> Patch(Guid id, JsonPatchDocument<Notification> updatedEntity)
        {
            await PatchNotification(id, updatedEntity);
            return true;
        }
        ...
        #region
        private async Task<List<Notification>> GetNotification(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            if (pageSize < 1)
            {
        ...
        private async Task<Guid> CreateNotification(Notification model)
        {
        ...
        private async Task UpdateNotification(Guid id, Notification updatedEntity)
        {
            _dbContext.Notification.Update(updatedEntity);
            await _dbContext.SaveChangesAsync();
        }

        private async Task<bool> DeleteNotification(Guid id)
        {
            var entityData = _dbContext.Notification.FirstOrDefault(entity => entity.Id == id);
            if (entityData == null)
            {
        ...
        private async Task PatchNotification(Guid id, JsonPatchDocument<Notification> updatedEntity)
        {
