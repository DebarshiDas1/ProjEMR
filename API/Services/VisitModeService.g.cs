...
namespace ProjEMR.Services
{
    ...
    public interface IVisitModeService
    {
        ...
        Task<List<VisitMode>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc");

        /// <summary>Adds a new visitmode</summary>
        /// <param name="model">The visitmode data to be added</param>
        /// <returns>The result of the operation</returns>
        Task<Guid> Create(VisitMode model);
        ...
        /// <returns>The result of the operation</returns>
        Task<bool> Update(Guid id, VisitMode updatedEntity);

        /// <summary>Updates a specific visitmode by its primary key</summary>
        /// <param name="id">The primary key of the visitmode</param>
        /// <param name="updatedEntity">The visitmode data to be updated</param>
        /// <returns>The result of the operation</returns>
        Task<bool> Patch(Guid id, JsonPatchDocument<VisitMode> updatedEntity);
    ...
    public class VisitModeService : IVisitModeService
    {
        ...
        public VisitModeService(ProjEMRContext dbContext, IFieldMapperService mapper)
        {
        ...
        public async Task<List<VisitMode>> Get(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            var result = await GetVisitMode(filters, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return result;
        }
        ...
        /// <param name="model">The visitmode data to be added</param>
        /// <returns>The result of the operation</returns>
        public async Task<Guid> Create(VisitMode model)
        {
            model.Id = await CreateVisitMode(model);
            return model.Id;
        }
        ...
        public async Task<bool> Update(Guid id, VisitMode updatedEntity)
        {
            await UpdateVisitMode(id, updatedEntity);
            return true;
        }
        ...
        /// <returns>The result of the operation</returns>
        /// <exception cref="Exception"></exception>
        public async Task<bool> Patch(Guid id, JsonPatchDocument<VisitMode> updatedEntity)
        {
            await PatchVisitMode(id, updatedEntity);
            return true;
        }
        ...
        {
            await DeleteVisitMode(id);
            return true;
        }
        #region
        private async Task<List<VisitMode>> GetVisitMode(List<FilterCriteria> filters = null, string searchTerm = "", int pageNumber = 1, int pageSize = 1, string sortField = null, string sortOrder = "asc")
        {
            if (pageSize < 1)
            {
        ...
        private async Task<Guid> CreateVisitMode(VisitMode model)
        {
        ...
        private async Task UpdateVisitMode(Guid id, VisitMode updatedEntity)
        {
            _dbContext.VisitMode.Update(updatedEntity);
            await _dbContext.SaveChangesAsync();
        }

        private async Task<bool> DeleteVisitMode(Guid id)
        {
        ...
        private async Task PatchVisitMode(Guid id, JsonPatchDocument<VisitMode> updatedEntity)
        {
