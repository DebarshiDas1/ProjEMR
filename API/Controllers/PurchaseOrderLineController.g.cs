...
namespace ProjEMR.Controllers
{
    ...
    [Authorize]
    public class PurchaseOrderLineController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] PurchaseOrderLine model)
        {
            model.TenantId = TenantId;
            var id = await _purchaseOrderLineService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _purchaseOrderLineService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _purchaseOrderLineService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _purchaseOrderLineService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] PurchaseOrderLine updatedEntity)
        {
            ...
            var status = await _purchaseOrderLineService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("PurchaseOrderLine", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<PurchaseOrderLine> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _purchaseOrderLineService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}