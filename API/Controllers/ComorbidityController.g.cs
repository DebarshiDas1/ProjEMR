...
namespace ProjEMR.Controllers
{
    ...
    [Authorize]
    public class ComorbidityController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] Comorbidity model)
        {
            model.TenantId = TenantId;
            var id = await _comorbidityService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _comorbidityService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _comorbidityService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _comorbidityService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] Comorbidity updatedEntity)
        {
            ...
            var status = await _comorbidityService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("Comorbidity", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<Comorbidity> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _comorbidityService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}