...
namespace ProjEMR.Controllers
{
    ...
    [Authorize]
    public class PatientNotesController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] PatientNotes model)
        {
            model.TenantId = TenantId;
            model.CreatedBy = UserId;
            model.CreatedOn = DateTime.UtcNow;
            var id = await _patientNotesService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _patientNotesService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _patientNotesService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _patientNotesService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] PatientNotes updatedEntity)
        {
            ...
            var status = await _patientNotesService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("PatientNotes", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<PatientNotes> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _patientNotesService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}