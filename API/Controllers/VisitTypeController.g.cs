...
namespace ProjEMR.Controllers
{
    ...
    public class VisitTypeController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] VisitType model)
        {
            model.TenantId = TenantId;
            var id = await _visitTypeService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _visitTypeService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _visitTypeService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _visitTypeService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] VisitType updatedEntity)
        {
            ...
            var status = await _visitTypeService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("VisitType", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<VisitType> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _visitTypeService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}