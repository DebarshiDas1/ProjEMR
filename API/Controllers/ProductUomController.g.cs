...
namespace ProjEMR.Controllers
{
    ...
    [Authorize]
    public class ProductUomController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] ProductUom model)
        {
            model.TenantId = TenantId;
            var id = await _productUomService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _productUomService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _productUomService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _productUomService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] ProductUom updatedEntity)
        {
            ...
            var status = await _productUomService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("ProductUom", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<ProductUom> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _productUomService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}