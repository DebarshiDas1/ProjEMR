...
namespace ProjEMR.Controllers
{
    ...
    [Authorize]
    public class VisitDiagnosisParameterController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] VisitDiagnosisParameter model)
        {
            model.TenantId = TenantId;
            var id = await _visitDiagnosisParameterService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _visitDiagnosisParameterService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _visitDiagnosisParameterService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _visitDiagnosisParameterService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] VisitDiagnosisParameter updatedEntity)
        {
            ...
            var status = await _visitDiagnosisParameterService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("VisitDiagnosisParameter", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<VisitDiagnosisParameter> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _visitDiagnosisParameterService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}