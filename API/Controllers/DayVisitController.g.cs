...
namespace ProjEMR.Controllers
{
    ...
    public class DayVisitController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] DayVisit model)
        {
            model.TenantId = TenantId;
            model.CreatedBy = UserId;
            model.CreatedOn = DateTime.UtcNow;
            var id = await _dayVisitService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _dayVisitService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _dayVisitService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _dayVisitService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] DayVisit updatedEntity)
        {
            ...
            var status = await _dayVisitService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("DayVisit", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<DayVisit> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _dayVisitService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}