...
namespace ProjEMR.Controllers
{
    ...
    [Authorize]
    public class PatientStatisticsController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] PatientStatistics model)
        {
            model.TenantId = TenantId;
            model.CreatedBy = UserId;
            model.CreatedOn = DateTime.UtcNow;
            var id = await _patientStatisticsService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _patientStatisticsService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _patientStatisticsService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _patientStatisticsService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] PatientStatistics updatedEntity)
        {
            ...
            var status = await _patientStatisticsService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("PatientStatistics", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<PatientStatistics> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _patientStatisticsService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}