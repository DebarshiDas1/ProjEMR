...
namespace ProjEMR.Controllers
{
    ...
    public class ProductClassificationController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] ProductClassification model)
        {
            model.TenantId = TenantId;
            model.CreatedBy = UserId;
            model.CreatedOn = DateTime.UtcNow;
            var id = await _productClassificationService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _productClassificationService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _productClassificationService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _productClassificationService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] ProductClassification updatedEntity)
        {
            ...
            var status = await _productClassificationService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("ProductClassification", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<ProductClassification> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _productClassificationService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}