...
namespace ProjEMR.Controllers
{
    ...
    public class VisitMedicalCertificateController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] VisitMedicalCertificate model)
        {
            model.TenantId = TenantId;
            model.CreatedBy = UserId;
            model.CreatedOn = DateTime.UtcNow;
            var id = await _visitMedicalCertificateService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _visitMedicalCertificateService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _visitMedicalCertificateService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _visitMedicalCertificateService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] VisitMedicalCertificate updatedEntity)
        {
            ...
            var status = await _visitMedicalCertificateService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("VisitMedicalCertificate", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<VisitMedicalCertificate> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _visitMedicalCertificateService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}