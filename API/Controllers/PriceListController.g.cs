...
namespace ProjEMR.Controllers
{
    ...
    public class PriceListController : BaseApiController
    {
        ...
        public async Task<IActionResult> Post([FromBody] PriceList model)
        {
            model.TenantId = TenantId;
            model.CreatedBy = UserId;
            model.CreatedOn = DateTime.UtcNow;
            var id = await _priceListService.Create(model);
            return Ok(new { id });
        }
        ...
        public async Task<IActionResult> Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = await _priceListService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = await _priceListService.GetById( id, fields);
            return Ok(result);
        }
        ...
        {
            var status = await _priceListService.Delete(id);
        ...
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] PriceList updatedEntity)
        {
            ...
            var status = await _priceListService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        [Produces("application/json")]
        [UserAuthorize("PriceList", Entitlements.Update)]
        public async Task<IActionResult> UpdateById(Guid id, [FromBody] JsonPatchDocument<PriceList> updatedEntity)
        {
            if (updatedEntity == null)
                return BadRequest("Patch document is missing.");
            var status = await _priceListService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}